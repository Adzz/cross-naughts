  ...
  describe "#end_state?" do
    it "returns true when X wins" do
      x_winning_combos.each do |combo|
        expect(Board.new(state: combo).end_state?).to be true
      end
    end
    ...
    it "returns true when there are no free spaces left" do
      expect(Board.new(state: ["X","O","X","O","X","O","O","X","X"]).end_state?).to be true
    end

    it "returns false when no one has one AND the board isn't full" do
      expect(Board.new(state: ["X","O","X","O","X","O",6,"X",8]).end_state?).to be false
    end
  end
